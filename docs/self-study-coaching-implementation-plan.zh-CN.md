...
- 当前只有 latest-signature 队列，没有 3 秒 debounce 和 15 秒自动限频
- 主动模式没有“同一作品、同一角色、已有有效提示时不重复请求”的保护
- 没有推荐结构的部分完成、整组完成和偏离方向判定
- 没有按作品维护的内存辅导上下文
- 角色切换、作品切换和退出时的上下文失效规则尚未实现
- 空白作品仍可能进入自动请求和本地起步推荐
