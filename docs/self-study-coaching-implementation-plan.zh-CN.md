...
- DeepSeek 只通过 Strict Tool Calls 返回受约束节点，客户端编译验证后生成 Scratch Blockly XML
...
- 已完成：学生端会把本地降级结果收口为通用短提示，不显示技术细节
...
- 已完成：`apps/desktop-companion/src/renderer/renderer-view.ts` 优先把结构化推荐渲染为一个连接/嵌套 workspace
- 已完成：推荐原因最多显示 3 条，按推荐结构遍历顺序对应，不强制补满
- 已完成：学生端只显示一句总提示，不显示模型来源、生成时间、额外下一步、示例、追问或诊断
- 已完成：学生端不直接展示 DeepSeek 原始错误，错误态使用通用短文案
- 已完成：推荐只读展示，不向 Scratch 自动插入或修改积木
...
状态：已完成。主界面已收口为一句总提示和一个只读推荐积木 workspace；结构化推荐按连接/嵌套关系整组渲染，最多展示 3 条简短原因，并隐藏模型名、生成时间、追问、示例、诊断和技术错误。
