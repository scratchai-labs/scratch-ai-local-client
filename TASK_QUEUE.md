...
- 2026-07-10：通过 `grill-with-docs` 压力测试单机直连版的自学辅导边界；已确认不建设自有教学服务器，学生使用自己的 AI API Key 直连第三方 AI 服务商；同时保留主动求助模式与自动辅导模式，保存有效 Key 后默认自动辅导，并在设置页向家长说明持续 API 调用与费用风险；仅在当前辅导会话内保留上一轮提示及后续作品变化，切换作品或退出客户端即清除且不落盘；自动辅导采用 3 秒静默窗口、15 秒最短自动请求间隔、单请求运行与最新状态追赶，主动点击不受自动间隔限制；面向低年级按建议使用顺序提供约 3 个关键积木，每个积木附一句简短原因，不展示额外示例或完整作品脚本；采用作品驱动、适应式与只读辅导，不提供聊天窗口、文字目标输入或自动积木写入；完全空白的作品不触发 AI，出现第一个有效积木后才开始辅导；学生偏离推荐后立即以新作品重新判断，不要求回退；保留内部辅导诊断但不向学生展示错误或警告列表；作品摘要保留当前角色完整脚本和其他角色少量摘要，不发送媒体资源或原始项目文件；AI 不可用时继续显示本地基础提示，学生端隐藏技术错误；继续压力测试实现边界。
