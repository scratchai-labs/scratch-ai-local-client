...
- 2026-07-10：通过 `grill-with-docs` 压力测试单机直连版的自学辅导边界；已确认不建设自有教学服务器，学生使用自己的 AI API Key 直连第三方 AI 服务商；同时保留主动求助模式与自动辅导模式；仅在当前辅导会话内保留上一轮提示及后续作品变化，切换作品或退出客户端即清除且不落盘；自动辅导采用 3 秒静默窗口、单请求运行与最新状态追赶；面向低年级默认直接提供 1–3 个关键积木，但不给完整作品脚本；继续确认局部示例与学生问题输入。
