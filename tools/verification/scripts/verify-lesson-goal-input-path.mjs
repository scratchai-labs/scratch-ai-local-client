/**
 * 本课目标输入联调：
 * 在主界面输入目标（如“鸡兔同笼”），再跑鸡兔同笼半成品路径，
 * 验证 DeepSeek/提示是否因 goal 更稳（减少任务反转/运动漂移）。
 *
 * 用法：
 *   node tools/verification/scripts/verify-lesson-goal-input-path.mjs
 *   node tools/verification/scripts/verify-lesson-goal-input-path.mjs --goal=鸡兔同笼 --follow-steps=3
 */
...
const usePackaged = argv.get('--packaged') === 'true';
const companionExe =
...
const companionCwd =
    ...
    (usePackaged
        ? path.dirname(companionExe)
        : path.join(workspaceRoot, 'apps', 'desktop-companion'));
...
const companionDebugPort = Number(argv.get('--port') ?? '9395');
...
const lessonGoal = String(argv.get('--goal') ?? '鸡兔同笼：已知头数和脚数，求鸡和兔各有多少，并说出来');

const artifactDir =
    argv.get('--artifact-dir') ??
    path.join(process.cwd(), 'lesson-goal-input-screenshots');
const userDataDir =
    argv.get('--user-data-dir') ??
    path.join(verificationRoot, 'tmp-lesson-goal-input-userdata');
...
function buildTypeLessonGoalExpression(goalText) {
    return `
(() => {
  const input = document.querySelector('#lesson-goal-input');
  if (!input) {
    return { ok: false, error: 'lesson-goal-input-missing' };
  }
  const value = ${JSON.stringify(goalText)};
...
async function main() {
    ...
    try {
        ...
        // 模拟学生/老师输入本课目标（可选；这里故意输入，验证效果）
        const typeGoal = await evaluateExpressionInTarget(mainTarget, buildTypeLessonGoalExpression(lessonGoal));
        const typeGoalValue = typeGoal.ok
            ? (typeof typeGoal.value === 'string' ? (() => { try { return JSON.parse(typeGoal.value); } catch { return typeGoal.value; } })() : typeGoal.value)
            : {ok: false, error: typeGoal.error};
        // wait for state sync of lessonGoal
        try {
        ...
        const lines = [
            ...
            '',
            '## Stages',
            '- goal-typed: free-text lesson goal entered',
